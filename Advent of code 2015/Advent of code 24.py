...
# Furthermore, Santa tells you, 
# if there are multiple ways to arrange the packages such that the fewest possible are in the first group, 
...
if mode == "input":
    input_path = "./Advent of code/input_day24.txt"
...
target = sum(parsed_data) / 4 # 516 kazda grupa
valid_groups = []

# for k in range(1 ,len(parsed_data)):
#     for group in itertools.combinations(parsed_data, k):
#         if sum(group) == target:
#             valid_groups.append(group)
#         if valid_groups: # jesli znalazles jakikolwiek valid group tzn ze to jest minimalna ilosc elementow jaka moze stworzyc taka grupe
#             break
# print(valid_groups)

for k in range(1 ,len(parsed_data)):
    for group in itertools.combinations(parsed_data, 5):
        if sum(group) == target:
            valid_groups.append(group)
        # if valid_groups: # jesli znalazles jakikolwiek valid group tzn ze to jest minimalna ilosc elementow jaka moze stworzyc taka grupe
        #     break
# print(valid_groups)
QE_list = []
for y in valid_groups:
    QE_list.append(math.prod(y))
    for z in QE_list:
        if math.prod(y) == min(QE_list):
            break
        break
    break
print(math.prod(y)) # wynik
